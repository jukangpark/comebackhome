...
### 결정됨
- 마이그레이션: **원시 SQL 파일 + `pg`** (부팅 시 `schema_migrations` 추적하며 순서 적용). ORM 없이 투명.
- 비밀번호: **8~72자**(bcrypt 72바이트 한계). username: **4~20자 영문/숫자/`_`**.
- JWT httpOnly 쿠키(SameSite lax, 7일). bcrypt cost 10. zod 검증.
- E2E 통과: 가입/me/중복(409)/오답(401)/로그아웃/재로그인/유효성(400).
...
### 결정됨
- 리사이즈: **sharp** (장변 1024 `fit:inside`, EXIF 회전보정, jpeg q88). 허용 jpg/png, 최대 10MB.
- 저장: 미디어 볼륨 `uploads/<petId>.jpg` (펫당 1장 upsert). 이미지 서빙은 `GET /api/pet/image`(본인만).
- 온보딩 재진입: 펫 없음→info, 펫만 있음→image, 완료→홈. (스텝은 P3/P4에서 확장)
- E2E 통과: 펫 생성/중복(409)/이미지 업로드/리사이즈(1600×1200→1024×768)/서빙.
...
### 결정됨
- 필드 분리: **traits(성격, 필수 1~500)** / **memories(추억, 필수 1~2000)** / **speaking(말투, 선택 0~200)**.
- 온보딩 마지막 단계로 편입(생성→persona→홈). `/persona` 편집 페이지 별도 제공.
- Home은 model DONE + hasPersona 둘 다여야 진입, 아니면 온보딩 해당 단계로 리다이렉트.
- E2E 통과: 404→저장(유효성 400 포함)→조회→hasPersona 전환→upsert 수정.
